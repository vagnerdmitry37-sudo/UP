    ...
    IOptions<AuthOptions> authOptions) : ITokenService
{
    ...
    {
        var tokenHash = _hcs.FindRequestCookieByKey(TokenNames.RefreshToken);
        if (tokenHash is null)
        {
            return null;
        }
        var encodedTokenHash = Convert.ToBase64String(SHA256.HashData(Encoding.UTF8.GetBytes(tokenHash)));
        return await _ar.FindRefreshTokenByHashAsync(encodedTokenHash);
    }
}
